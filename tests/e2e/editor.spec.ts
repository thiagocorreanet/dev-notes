...
test('previews, merges and restores a workspace backup without touching the original file', async ({
  page,
  localFile,
}) => {
  const backup = {
    ...
    folders: [{ id: 'folder', name: 'Recovered folder' }],
    notes: [
      {
        id: 'copy',
  ...
  await page
    .getByRole('button', { name: 'Importar backup', exact: true })
    .click()
  await page.getByLabel('Arquivo de backup', { exact: true }).setInputFiles({
    name: 'backup.json',
    mimeType: 'application/json',
    buffer: Buffer.from(JSON.stringify(backup)),
  })
  await expect(page.getByLabel('Prévia do backup')).toContainText(
    'Imported workspace',
  )
  await page
    .getByRole('button', { name: 'Mesclar backup', exact: true })
    .click()
  await expect(
    page.getByRole('heading', { name: 'Recovered note', exact: true }),
  ).toBeVisible()
  await expect(
    page.getByRole('region', { name: 'Local de salvamento' }),
  ).toContainText('Salvar guarda neste navegador')
  await page
    .getByRole('button', { name: 'Importar backup', exact: true })
    .click()
  await page.getByLabel('Arquivo de backup', { exact: true }).setInputFiles({
  ...
  await page.getByRole('combobox', { name: 'Como importar' }).click()
  await page
    .getByRole('option', { name: 'Restaurar o espaço de trabalho' })
    .click()
  await expect(
    page.getByRole('button', { name: 'Restaurar backup', exact: true }),
  ).toBeDisabled()
  ...
  const saved = await page.evaluate(
    () =>
      JSON.parse(localStorage.getItem('dev-notes:workspace:v1')!) as {
        notes: unknown[]
        folders: unknown[]
      },
  )
  ...
  await page.reload()
  await page
    .getByRole('button', { name: 'Pasta: Recovered folder', exact: true })
    .click()
  await page
    .getByRole('button', { name: 'Recovered note', exact: true })
    .first()
    .click()
  await expect(
    page.getByRole('heading', { name: 'Recovered note', exact: true }),
